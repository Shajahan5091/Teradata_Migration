...
BEGIN
	DECLARE result_cursor CURSOR WITH RETURN ONLY FOR
		SELECT
			...
			q.SalesAmountQuota
		FROM
			AdventureWorksDW.FactSalesQuota q
		JOIN
			AdventureWorksDW.DimEmployee e ON q.EmployeeKey = e.EmployeeKey
		ORDER BY
			q.CalendarYear,
			q.CalendarQuarter,
			EmployeeName;
...
BEGIN
	DECLARE result_cursor CURSOR WITH RETURN ONLY FOR
		...
		FROM
			AdventureWorksDW.FactResellerSales s
		JOIN
			AdventureWorksDW.DimReseller r
			ON s.ResellerKey = r.ResellerKey
		JOIN
			AdventureWorksDW.DimGeography geo ON r.GeographyKey = geo.GeographyKey
		GROUP BY
